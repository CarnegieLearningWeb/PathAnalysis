...
export const countEdges = (
    ...
    const top5Sequences = getTopSequences(stepSequences, 10)
...
//         .sort(([, countA], [, countB]) => countB - countA)
//         .slice(0, topN);
//     console.log("Sorted w counts: " + sortedSequences)
//     console.log(sortedSequences.map(([sequenceKey, count]) => JSON.parse(sequenceKey, sequenceCounts[sequenceKey])))
//     const topSequences = sortedSequences.map(([sequenceKey, count]) => JSON.parse(sequenceKey, sequenceCounts[sequenceKey]));
//     return topSequences;
// }
//
// interface EdgeCounts {
...
//
// // Function to calculate the color of a node based on its rank in the most common sequence
// function calculateColor(sequence: string[], rank: number, totalSteps: number): string {
//     const ratio = rank / totalSteps;
//
//     const white = {r: 255, g: 255, b: 255};
//     const lightBlue = {r: 0, g: 166, b: 255};
...
//     // Create node definitions in the DOT string
//     let dotString = 'digraph G {\n';
//     Object.keys(edgeCounts).forEach((sourceNode) => {
//         // Determine the rank of the node in the selected sequence
//         const rank = selectedSequence.indexOf(sourceNode) + 1;
//         const color = rank > 0 ? calculateColor(selectedSequence, rank, totalSteps) : '#FFFFFF'; // Default to white if not in sequence
//
//         // dotString += `"${sourceNode}" [style=filled, fillcolor="${color}"];\n`;
//
//         // for (let rank = 0; rank < totalSteps; rank++) {
//         //     const step = stepsInSelectedSequence[rank];
//         //     const color = calculateColor(selectedSequence, rank, totalSteps);
//         //     console.log(color, step)
//         const node_tooltip = `Rank:\n\t\t ${rank + 1}\nColor:\n\t\t ${color}`;
//
//         dotString += `    "${sourceNode}" [rank=${rank + 1}, style=filled, fillcolor="${color}", tooltip="${node_tooltip}"];\n`;
//     }
//
//         // Create edge definitions in the DOT string based on normalized thickness and thresholds
//         for (const edge of Object.keys(normalizedThicknesses))
